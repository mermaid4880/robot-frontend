...
function HkwsHD(props) {
  ...
  function destroyIframe() {
    ...
    console.log("after parent.document.body.removeChild(el)");
  }
  ...
  function eventHandler(event) {
    ...
    //停止预览并登出
      ...
      console.log("我是iframeHD，我收到了：2");
      hkwsStopAndLogout(); //停止预览并登出
    }
    //拍照
    else if (event.data === 3) {
      console.log("我是iframeHD，我收到了：3");
      hkwsCapturePic(); //拍照
    }
    //开始录像
    else if (event.data === 4) {
      console.log("我是iframeHD，我收到了：4");
      hkwsStartRecord(); //开始录像
    }
    //停止录像
    else if (event.data === 5) {
      console.log("我是iframeHD，我收到了：5");
      hkwsStopRecord(); //停止录像
    }
    //开始对讲
    else if (event.data === 6) {
      console.log("我是iframeHD，我收到了：6");
      hkwsStartVoiceTalk(); //开始录像
    }
    //停止对讲
    else if (event.data === 7) {
      console.log("我是iframeHD，我收到了：7");
      hkwsStopVoiceTalk(); //停止录像
    }
    //销毁iframeHD
    else if (event.data === 0) {
      console.log("我是iframeHD，我收到了：0");
      destroyIframe(); //销毁iframeHD
    }
    //停止预览并登出、销毁iframeHD
    else if (event.data === 20) {
      console.log("我是iframeHD，我收到了：20");
      hkwsStopAndLogout(); //停止预览并登出
      destroyIframe(); //销毁iframeHD
    }
  }
