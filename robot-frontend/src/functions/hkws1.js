...
import {
  WebVideoCtrl,
  mWebVideoCtrl,
} from "../../HKWS/codebase/webVideoCtrl.js";
import $ from "jquery";

// 全局保存当前选中窗口
var g_iWndIndex = 0; //可以不用设置这个变量，有窗口参数的接口中，不用传值，开发包会默认使用当前选择窗口
function HkwsStart(cameraIP) {
  ...
  var oPlugin = {
    iWidth: 900, // plugin width
    iHeight: 600, // plugin height
  };

  var oLiveView = {
    iProtocol: 1, // protocol 1：http, 2:https
    szIP: cameraIP,//"192.168.1.65", // protocol ip 高清
    // szIP_2: "192.168.1.40", // protocol ip 模拟
  ...
  WebVideoCtrl.I_InitPlugin(oPlugin.iWidth, oPlugin.iHeight, {
    ...
    cbInitPluginComplete: function () {
      ...
      // }
      if (g_iWndIndex === 2) {
        oLiveView.szIP = "192.168.2.13";
      }

      console.log(oLiveView.szIP);
...
function HkwsStart1() {
  ...
  var oPlugin = {
    iWidth: 450, // plugin width
    iHeight: 300, // plugin height
  };

  var oLiveView = {
    iProtocol: 1, // protocol 1：http, 2:https
    // szIP: "192.168.1.64", // protocol ip 高清
    szIP: "192.168.1.40", // protocol ip 模拟
    szPort: "80", // protocol port
    szUsername: "admin", // device username
    szPassword: "zngdzx613", // device password
    iStreamType: 1, // stream 1：main stream  2：sub-stream  3：third stream  4：transcode stream
...
//登出
function HkwsLogout(cameraIP) {
  console.log("enter Logout");
  var oLiveView = {
    iProtocol: 1, // protocol 1：http, 2:https
    szIP: cameraIP,//"192.168.1.65", // protocol ip 高清
    // szIP: "192.168.2.13", // protocol ip 模拟
    szPort: "80", // protocol port
    szUsername: "admin", // device username
    szPassword: "zngdzx613", // device password
    iStreamType: 1, // stream 1：main stream  2：sub-stream  3：third stream  4：transcode stream
  ...
  var szDeviceIdentify = oLiveView.szIP + "_" + oLiveView.szPort,
    szDeviceIdentify1 = "192.168.2.13" + "_" + oLiveView.szPort,
    szInfo = "";
  ...
  var iRet =
    ...
    WebVideoCtrl.I_Logout(szDeviceIdentify1);
  if (0 == iRet) {
    szInfo = "退出成功！";
    getChannelInfo();
    getDevicePort();
...
function HkwsRealPlay() {
  console.log("enter HkwsRealPlay");
  var oWndInfo = WebVideoCtrl.I_GetWindowStatus(g_iWndIndex),
...
//开始录像
function HkwsStartRecord() {
  var oWndInfo = WebVideoCtrl.I_GetWindowStatus(g_iWndIndex),
    szFileName = "",
    szInfo = "";
  if (g_iWndIndex === 0) {
    szFileName = "高清录像" + "_" + new Date().getTime();
  } else if (g_iWndIndex === 2) {
    szFileName = "红外录像" + "_" + new Date().getTime();
  }
  if (oWndInfo != null) {
    //   var szChannelID = "1",
    //     szFileName =
...
// 停止录像
function HkwsStopRecord(iWndIndex) {
  // alert("stop");
...
// 开始对讲
function HkwsStartVoiceTalk() {
  var szDeviceIdentify = "192.168.1.64";

  // if (g_iWndIndex === 0) {
  //   szDeviceIdentify = "192.168.1.64";
  // } else if (g_iWndIndex === 2) {
  //   szDeviceIdentify = "192.168.2.13";
  // }
...
//本地配置
function HkwsSetLocalCfg() {
  var arrXml = [],
    dir = "C:/Users/613-ROBOT-1/Desktop/FrontEndFiles",
    szInfo = "";
  ...
  arrXml.push("<RecordPath>" + dir + "</RecordPath>");
  arrXml.push("<CapturePath>" + dir + "</CapturePath>");
  arrXml.push("<PlaybackFilePath>" + dir + "</PlaybackFilePath>");
  arrXml.push("<PlaybackPicPath>" + dir + "</PlaybackPicPath>");
  arrXml.push("<DeviceCapturePath>" + dir + "</DeviceCapturePath>");
  arrXml.push("<DownloadPath>" + dir + "</DownloadPath>");
...
function clickStartRealPlay(iStreamType) {
  console.log("enter clickStartRealPlay");
  var oWndInfo = WebVideoCtrl.I_GetWindowStatus(g_iWndIndex),
    szDeviceIdentify = $("#ip").val(),
    iRtspPort = parseInt($("#rtspport").val(), 10),
    iChannelID = parseInt($("#channels").val(), 10),
