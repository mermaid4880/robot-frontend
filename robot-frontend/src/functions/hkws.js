...
function hkwsInit(width, height) {
  ...
  arrXml.push("<LocalConfigInfo>");
  arrXml.push("<PackgeSize>" + "0" + "</PackgeSize>"); //录像文件打包大小
  arrXml.push("<PlayWndType>" + "0" + "</PlayWndType>"); //图像尺寸
  arrXml.push("<BuffNumberType>" + "0" + "</BuffNumberType>"); //播放性能
  arrXml.push("<RecordPath>" + recordPath + "</RecordPath>"); //录像文件保存路径
  arrXml.push("<CapturePath>" + recordPath + "</CapturePath>"); //预览抓图保存路径
  arrXml.push("<PlaybackFilePath>" + recordPath + "</PlaybackFilePath>"); //回放剪辑保存路径
  arrXml.push("<PlaybackPicPath>" + recordPath + "</PlaybackPicPath>"); //回放抓图保存路径
  arrXml.push("<DeviceCapturePath>" + recordPath + "</DeviceCapturePath>"); //设备抓图保存路径
  arrXml.push("<DownloadPath>" + recordPath + "</DownloadPath>"); //回放下载保存路径
  arrXml.push("<IVSMode>" + "1" + "</IVSMode>"); //规则信息
  arrXml.push("<CaptureFileFormat>" + "1" + "</CaptureFileFormat>"); //抓图文件格式
  arrXml.push("<ProtocolType>" + "0" + "</ProtocolType>"); //协议类型
  arrXml.push("</LocalConfigInfo>");
...
function startPlay() {
  var oWndInfo = WebVideoCtrl.I_GetWindowStatus(g_iWndIndex),
    //szDeviceIdentify = IP, // protocol ip
...
//停止预览并登出
function hkwsStopAndLogout() {
  var oWndInfo = WebVideoCtrl.I_GetWindowStatus(g_iWndIndex),
    szInfo = "";

  console.log("stopPlay()  oWndInfo", oWndInfo);
  if (oWndInfo != null) {
    WebVideoCtrl.I_Stop({
      success: function () {
        szInfo = "停止预览成功！";
...
function hkwsStartRecord() {
  ...
  g_szRecordType = "realplay";

  if (oWndInfo != null) {
    var szChannelID = 1, //$("#channels").val(),
      szFileName =
        oWndInfo.szDeviceIdentify +
        "_" +
        szChannelID +
        "_" +
        new Date().getTime();
    showOPInfo(oWndInfo.szDeviceIdentify + " " + szFileName);
    WebVideoCtrl.I_StartRecord(szFileName, {
      bDateDir: true, //是否生成日期文件
      success: function () {
        szInfo = "开始录像成功！";
...
export {
  hkwsInit, //初始化插件、设置本地配置、设置窗口分割数（窗口宽度，窗口高度）
  hkwsLoginAndStart, //登录并开始预览（设备IP）
  hkwsStopAndLogout, //停止预览并登出
  hkwsCapturePic, //拍照
  hkwsStartRecord, //开始录像
  hkwsStopRecord, //停止录像
  hkwsStartVoiceTalk, //开始对讲
  hkwsStopVoiceTalk, //停止对讲
};
